...
Msty Studio supports local MCP server that run on your machine as well as remote MCP servers that can be connect to over HTTP. 
...
1. Select **HTTP** for remote MCP servers and tools or **STDIO / JSON** for local MCP servers and tools.
