---
title: Lost & Found
description: Discover and recover lost or orphaned workspaces using the Lost & Found feature in Msty Studio.
navTruncate: false
resources:
  videos:
  blogs:
...
For privacy and cross-compatibility reasons, Msty stores your data locally in an OPFS database [Read our blog post on how this ensures privacy](https://msty.ai/blog/msty-studio-web-opfs). Very rarely, your browser may create a new database unexpectedly. When that happens, your previous workspaces may not appear in the app but **Lost & Found** can help you find and recover them easily.
...
#### Web
- **Scan Folder** – select a folder using the browser’s directory picker or file-input fallback
::

### Scanning Browser File System Data (Web)
...
::steps
1. Find the directory on your computer where your browser stores its `File System` data.
...
You can now scan and recover workspaces without interruption. After scanning, a table lists all valid workspace databases.

Each row shows:
- Workspace name  
- File name + full path (via tooltip)  
- File size  
- Last modified date  
- Status (Current, Linked, or Found)

::alert{type="info" icon="tabler:info-circle"}
Only actual Msty workspace databases are shown.
::
...
::alert{type="info" icon="tabler:info-circle"}
The feature never reads files outside the app’s storage or outside folders explicitly selected by the user.
::
