...
If you select multiple stacks to be available in a conversation, then the default settings for one of the stack will be returned. So, it is recommended to validate and configure the query settings to be used for the conversation. 
...
#### Pull Mode
This mode allows for the model to perform the actual query. In essence, this makes Knowledge Stacks act more like an MCP tool that a model is able to leverage. 
...
::alert{type="info" icon="tabler:info-circle"}
Note: This is disabled by default and should be used selectively based on the task and model token limits.
::
