...
To utilize the Toolbox features, you must have [Sidecar](/getting-started/sidecar) running and Studio connected.
...
To get started, you can select option to **Import Default Tools**. Please keep in mind these tools are not created nor maintained by Msty team and are meant to provide a starting point.
...
If using the above example, take note of the variables in the mustache brackets. These are the variables that will be passed to the tool when it is ran. You can use these variables in your tool configuration to make it more dynamic. You will be able to define variables in the following steps.
...
To set default parameters for a tool, select the **\*** icon next to the tool name. This will open a window where you can define the tool's parameters.
