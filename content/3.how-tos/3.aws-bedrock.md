...
5. Grab the **AWS Region** the model is being enabled in
...
The [aws cli](https://aws.amazon.com/cli/) can be used for this purpose by running the following command: 
...
7. Grab the IAM user's 7. Grab the IAM user's **Access Key ID** and **Secret Access Key** 
...
Remember to grab the IAM user's Access Key ID, Secret Access Key, and Session Token if using short-term access.  
