...
The Enterprise plan is designed for teams and organizations that required advanced features, including user management, collaboration tools, audit logs, and additional security features.

To learn more about the Enterprise plan and to get pricing information and request access to a pilot,  please visit the [Msty Studio Enterprise page](https://msty.studio/enterprise).
...
Msty prides ourselves with leaving data where it belongs, in your control and not ours. We take a hands-off approach to user data and conversations while ensuring that we provide a secure and reliable platform for our users.

- **Conversation Data**: Remains on user devices and is not stored on Msty servers. 
    - Any external online models used, such as ChatGPT, Gemini, Cluade, etc will be provided conversation and context data as they are external LLM providers. Please refer to their respective privacy policies for more information. 
    - MCP Tools that connect to external services (such as web search, file retrieval, etc) will also provide data to those external services as needed. Please refer to their respective privacy policies for more information.
